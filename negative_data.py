...
def spore_rectangles(spore_table, n_spores, max_length):
    ...
    while spored < n_spores:
        ...
        x_range = np.linspace(prop_x - idx_length,
                              prop_x + idx_length,
                              2*idx_length + 1).astype(int)

        y_range = np.linspace(prop_y - idx_length,
                              prop_y + idx_length,
                              2*idx_length + 1).astype(int)
...
def map_spores(growing_table, spore_table, spore_length, spore_width, max_length):
